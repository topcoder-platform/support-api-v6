...
const SLACK_BODY_PREVIEW_CHARACTERS = 1_000;
...
export class NotificationOutboxService {
  ...
  /**
   * Queues a Slack intent announcing that a Support Team member took a ticket,
   * so the rest of the team sees the assignment without polling the queue.
   * The caller-generated assignment event ID keeps repeated
   * assign/unassign/assign cycles individually notifiable while making each
   * cycle's delivery retry-idempotent.
   *
   * @param tx transaction that created the assignment.
   * @param ticketId assigned ticket ID.
   * @param assigneeHandle handle snapshot of the newly assigned member.
   * @param assignEventId unique discriminator generated by the winning assignment.
   * @returns queued or existing outbox IDs.
   * @throws Prisma errors so the caller's domain transaction rolls back atomically.
   */
  async queueTicketAssigned(
    tx: Prisma.TransactionClient,
    ticketId: string,
    assigneeHandle: string,
    ...
    return this.queueIntents(tx, [
      {
        dedupeKey: `ticket:${ticketId}:assigned:${assignEventId}:slack`,
        ticketId,
        assigneeHandle,
        channel: NotificationChannel.SLACK,
   ...
   * @throws Slack, missing-assignee, or unsupported-event errors.
   */
  private async deliverSlack(record: NotificationRecord): Promise<void> {
    ...
    if (record.type === NotificationType.TICKET_ASSIGNED) {
      if (!record.assigneeHandle) {
        throw new IntegrationDeliveryError('assignee_handle_missing');
      }
      await this.slack.sendNotification(
        this.slackMessage(
          record,
          `Support ticket for ${handle} was assigned to ` +
            `${this.escapeSlack(record.assigneeHandle)}.`,
        ),
      );
      return;
    }
    if (record.type === NotificationType.TICKET_CLOSED) {
      await this.slack.sendNotification(
        this.slackMessage(record, `Support ticket for ${handle} was closed.`),
      );
      return;
    }
    if (record.type === NotificationType.TICKET_REOPENED) {
      await this.slack.sendNotification(
        this.slackMessage(record, `Support ticket was reopened by ${handle}.`),
      );
      return;
    }
  ...
  /**
   * Assembles one multi-line Slack message from an event headline, the shared
   * challenge and ticket context lines, and optional trailing detail lines.
   *
   * @param record claimed outbox row with ticket data.
  ...
  private slackMessage(
    ...
    if (record.ticket.challengeId) {
      lines.push(`Challenge: ${this.challengeLink(record.ticket.challengeId)}`);
    }
    lines.push(`Ticket: ${this.ticketUrl(record.ticketId)}`, ...detailLines);
    return lines.join('\n');
  }

  /**
   * Builds a Slack `mrkdwn` link pointing at the challenge in the Work app.
   *
   * @param challengeId challenge identifier stored on the ticket.
   * @returns Slack link whose label is the escaped challenge ID.
   */
  private challengeLink(challengeId: string): string {
    const base =
      this.config.get<string>('CHALLENGE_APP_BASE_URL')?.trim() ||
      DEFAULT_CHALLENGE_APP_BASE_URL;
    const url = `${base.replace(/\/+$/, '')}/challenges/${encodeURIComponent(
      challengeId,
    )}`;
    return `<${url}|${this.escapeSlack(challengeId)}>`;
  }
