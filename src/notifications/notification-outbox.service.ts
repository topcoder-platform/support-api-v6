...
export class NotificationOutboxService {
   ...
   * Duplicate close handling reuses the unique intent rows.
   *
  ...
  async queueTicketClosed(
    ...
    return this.queueIntents(tx, [
      {
        dedupeKey: `ticket:${ticketId}:closed:email`,
        ticketId,
        channel: NotificationChannel.EMAIL,
        type: NotificationType.TICKET_CLOSED,
      },
      {
        dedupeKey: `ticket:${ticketId}:closed:slack`,
        ticketId,
        channel: NotificationChannel.SLACK,
        type: NotificationType.TICKET_CLOSED,
      },
  ...
  private async deliverClaim(claim: ClaimedNotification): Promise<void> {
    ...
    try {
      ...
      await this.recordDeliveryFailure(record, claim.lockedAt);
    }
  }
  ...
  private async deliverEmail(record: NotificationRecord): Promise<void> {
    ...
    if (!member.email) {
      throw new Error('Ticket member does not have a notification email.');
    }
    if (record.type === NotificationType.TICKET_REPLIED) {
      if (
        ...
        throw new Error('Reply notification does not have a support response.');
      }
    ...
    if (record.type === NotificationType.TICKET_CLOSED) {
      await this.eventBus.sendSupportEmail('closed', [member.email], {
        ...
        closedAt: record.ticket.closedAt?.toISOString() ?? '',
      });
      return;
    }
    throw new Error('Unsupported support email type.');
  }
  ...
  private async deliverSlack(record: NotificationRecord): Promise<void> {
    ...
    throw new Error('Unsupported support Slack notification type.');
  }
  ...
  private async recordDeliveryFailure(
    ...
    await this.db.notificationOutbox.updateMany({
      ...
      data: {
        ...
        lockedAt: null,
        lastError: exhausted
          ? 'Notification delivery exhausted its retry limit.'
          : 'Notification delivery failed and will be retried.',
      },
    });
    this.logger.warn(
      `Support notification ${record.id} ${exhausted ? 'is dead' : 'will retry'}.`,
    );
  }
